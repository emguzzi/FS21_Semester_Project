...
#########################
# tensorflow.contrib was removed in versions > 1.14
#########################
...
def network(batch_size,precision,resolution,in_dim,h_dim, out_dim,model_in_vars=None, model_in_shapes=None, trainable_model_in='True'):
    ...
    #ADDED: tf.int64 instead of prec + name = 'corrupt_prob' + cast line
    corrupt_prob = tf.placeholder(tf.int64,[1],name = 'corrupt_prob')
    corrupt_prob = tf.cast(corrupt_prob,prec)
    ...
    #     cnv3_out = net
        ...
        #removed this line because not working with gpu
    ...
    ################################
    # Modified! original version above
    #################################
    #consider only the layer relative to the automap network and remove the prefix of the var name 
    #i.e. gen_AUTOMAP/automap_layer_1/ (28 chars)
    #print(tf.trainable_variables())
    #vars = [var for var in tf.trainable_variables() if 'gen_AUTOMAP/automap_layer_1/' in var.name]
    #vars = []
    #for var in tf.trainable_variables():
    #    if 'gen_AUTOMAP' in var.name:
    #        var.name = var.name[0:28]
    #        vars.append(var)
    #print(vars)
    #for var in range(len(vars)):
    #    paramtuple_list.append(vars[var].name.split('/')[-1].split(':')[0])
    #    param_data_list.append(vars[var])
    #print(paramtuple_list)
    ...
    # change its' rotation.
    ycrop = tf.transpose(ycrop, perm=[0,2,1],name='ycrop');
