...
import numpy as np;
from config import src_weights, src_mri_data, \
...
def compile_robust_network(sess, batch_size, src_weights=src_weights):
    
    model_in_vars =  ['W1_fc', 'b1_fc', 'W2_fc', 'b2_fc', 'W1_cnv', 'b1_cnv', 'W2_cnv', 'b2_cnv', 'W1_dcv', 'b1_dcv'];
    [W1_fc, b1_fc, W2_fc, b2_fc, W1_cnv, b1_cnv, W2_cnv, b2_cnv, W1_dcv, b1_dcv] = [np.load(os.path.join(src_weights,file+'.npy')) for file in model_in_vars]
    model_in_shape = [W1_fc.shape, b1_fc.shape, 
                      W2_fc.shape, b2_fc.shape, 
                      W1_cnv.shape, b1_cnv.shape, 
                      W2_cnv.shape, b2_cnv.shape,
